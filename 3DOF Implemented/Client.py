...
# Convert server data to a matrix
...
# Construct object sides from its vertices
...
# Connect to server, track object orientation using received server data
def main():

	# Setup client<>server communication line
	host = '192.168.1.87' # Fixed RPi IP address
	port = 5560 # Free port on laptop
	...
	while True:
		...
		# Plot new orientation from received vertices
		elif data:
			sys.clear()
			try:
			...
			plt.draw()
			plt.pause(0.001) #IF THIS IS TOO LONG OR SHORT, RECEIVED DATA WILL HAVE EXTRA/MISSING PACKAGES AND PROGRAM WILL CRASH
